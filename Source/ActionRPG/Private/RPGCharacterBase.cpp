...
void ARPGCharacterBase::AddStartupGameplayAbilities()
{
	...
	if (GetLocalRole() == ROLE_Authority && !bAbilitiesInitialized)
	{
		...
		for (TSubclassOf<URPGGameplayAbility>& StartupAbility : GameplayAbilities)
		{
			...
			UE_LOG(LearningLog, Warning, TEXT("Grant [%s] ability, but only on the server!"), *StartupAbility->GetName());
		...
		for (TSubclassOf<UGameplayEffect>& GameplayEffect : PassiveGameplayEffects)
		{
			...
			if (NewHandle.IsValid())
			{
				...
				UE_LOG(LearningLog, Warning, TEXT("FunctionName: %s. Now apply [%s] effect of passives only on the server!"), *FString(__FUNCTION__), *GameplayEffect->GetName());
			}
		}
...
void ARPGCharacterBase::FillSlottedAbilitySpecs(TMap<FRPGItemSlot, FGameplayAbilitySpec>& SlottedAbilitySpecs)
{
	...
	for (const TPair<FRPGItemSlot, TSubclassOf<URPGGameplayAbility>>& DefaultPair : DefaultSlottedAbilities)
	{
		if (DefaultPair.Value.Get())
		{
			...
			UE_LOG(LearningLog, Warning, TEXT("FunctionName: %s. SlottedAbilitySpecs Add GA_Spec"), *FString(__FUNCTION__));
	...
	if (InventorySource)
	{
		...
		for (const TPair<FRPGItemSlot, URPGItem*>& ItemPair : SlottedItemMap)
		{
			...
			if (SlottedItem && SlottedItem->GrantedAbility)
			{
				...
				UE_LOG(LearningLog, Warning, TEXT("Function Name: %s. Ablility Name: %s"), *FString(__FUNCTION__), *(SlottedItem->GrantedAbility->GetName()));
			}
		}
	}
}
...
void ARPGCharacterBase::AddSlottedGameplayAbilities()
{
	...
	for (const TPair<FRPGItemSlot, FGameplayAbilitySpec>& SpecPair : SlottedAbilitySpecs)
	{
		...
		if (!SpecHandle.IsValid())
		{
			...
			UE_LOG(LearningLog, Warning, TEXT("Function Name: %s. \
				Item Type: %s. \
				Handle: %s. \
				Address: %x. \
				FindAddress: %x."), 
				* FString(__FUNCTION__), 
				*(SpecPair.Key.ItemType.GetName().ToString()), 
				*(SpecHandle.ToString()), 
				&SpecHandle,
				SlottedAbilities.Find(SpecPair.Key));
...
bool ARPGCharacterBase::ActivateAbilitiesWithItemSlot(FRPGItemSlot ItemSlot, bool bAllowRemoteActivation)
{
	...
	{
		UE_LOG(LearningLog, Warning, TEXT("Function Name: %s. TryActivateAbility FoundHandle %s."), *FString(__FUNCTION__), *(FoundHandle->ToString()));
