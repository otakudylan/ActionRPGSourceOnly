...
bool ARPGPlayerControllerBase::LoadInventory()
{
	...
	for (const TPair<FPrimaryAssetType, int32>& Pair : GameInstance->ItemSlotsPerType)
	{
		for (int32 SlotNumber = 0; SlotNumber < Pair.Value; SlotNumber++)
		{
			...
			UE_LOG(LearningLog, Warning, TEXT("Function Name: %s, Information: %s %d"), *FString(__FUNCTION__),*Pair.Key.GetName().ToString(), SlotNumber);
		}
	}
	...
	if (CurrentSaveGame)
	{
		UE_LOG(LearningLog, Warning, TEXT("Function Name: %s, CurrentSaveGame is not null."), *FString(__FUNCTION__));
		...
		for (const TPair<FPrimaryAssetId, FRPGItemData>& ItemPair : CurrentSaveGame->InventoryData)
		{
			...
			if (LoadedItem != nullptr)
			{
				...
				UE_LOG(LearningLog, Warning, TEXT("Function Name: %s, InventoryData Add: %s."), *FString(__FUNCTION__), *(LoadedItem->ItemName.ToString()));
		...
		for (const TPair<FRPGItemSlot, FPrimaryAssetId>& SlotPair : CurrentSaveGame->SlottedItems)
		{
			if (SlotPair.Value.IsValid())
			{
				...
				if (GameInstance->IsValidItemSlot(SlotPair.Key) && LoadedItem)
				{
					...
					bFoundAnySlots = true;
					UE_LOG(LearningLog, Warning, TEXT("Function Name: %s, SlottedItems Add: %s."), *FString(__FUNCTION__), *(LoadedItem->ItemName.ToString()));
				}
			}
		}
		...
		if (!bFoundAnySlots)
		{
			UE_LOG(LearningLog, Warning, TEXT("Function Name: %s, will FillEmptySlots."), *FString(__FUNCTION__));
...
bool ARPGPlayerControllerBase::FillEmptySlotWithItem(URPGItem* NewItem)
{
	...
	for (TPair<FRPGItemSlot, URPGItem*>& Pair : SlottedItems)
	{
		UE_LOG(LearningLog, Warning, TEXT("FunctionName: %s. SlottedItem is not empty!"), *FString(__FUNCTION__));
